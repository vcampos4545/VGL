...
{
  if (!m_vao || !m_isLineMode || m_vertexCount < 2)
    return;
  glBindVertexArray(m_vao);
...
namespace MeshGen
{
  ...
  void cube(std::vector<Vertex> &vertices, std::vector<unsigned int> &indices)
  {
    ...
    for (int face = 0; face < 6; ++face)
    {
      ...
      for (int v = 0; v < 4; ++v)
      {
        vertices.push_back({{positions[face][v][0], positions[face][v][1], positions[face][v][2]},
                            normals[face],
                            {uvs[v][0], uvs[v][1]}});
      }
  ...
  void sphere(std::vector<Vertex> &vertices, std::vector<unsigned int> &indices, int rings, int sectors)
  {
    ...
    for (int r = 0; r <= rings; ++r)
    {
      ...
      for (int s = 0; s <= sectors; ++s)
      {
        ...
        glm::vec3 pos(x, y, z);
        vertices.push_back({pos * 0.5f, // radius 0.5 so diameter = 1 (unit sphere)
                            glm::normalize(pos),
